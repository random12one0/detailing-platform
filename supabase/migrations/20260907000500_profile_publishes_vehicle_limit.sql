-- ROADMAP 8.10 — THE BOOKING PAGE HAS TO BE TOLD HOW MANY CARS ARE ALLOWED.
--
-- `get_public_business_profile` publishes settings as an EXPLICIT key list
-- rather than `to_jsonb(s)`, so a new column is invisible to every booking
-- form in the world until it is named here. That is deliberate — this is the
-- whole public read surface and a settings column is not automatically public
-- — and it is also contract §2's failure shape exactly: the dashboard setting
-- saves, the screen works, and the feature reaches nobody.
--
-- Only the LIMIT is published. `extra_vehicle_minutes_saved` stays private:
-- the duration a customer is shown comes from `calculate-booking`, so a form
-- has nothing to do with the number and publishing it would invite one to
-- compute its own.
--
-- The body below is the LIVE definition with one line added. Nothing else in
-- it changed.
