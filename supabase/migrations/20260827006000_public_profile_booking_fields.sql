-- The public booking page needs a few more public-safe fields than the
-- original profile RPC returned: the drop-off address and phone (shown when
-- a customer picks drop-off, or needs to call), the service area, and the
-- booking rules that shape the form (travel fee, minimum notice, vehicle
-- sizes are implicit in each service's adjustments).
--
-- Still strictly public-safe: no customer data, no revenue, no internal
-- settings, and still exactly one business resolved by slug.
