...
Multi-tenant SaaS that gives an independent detailer a professional website with online booking built in. Converted from a single-business site (Andrew's Auto Detail), which still operates and is off-limits. Success right now means **launch-ready polish**: closing the open threads before seeking users, not growth metrics yet. (The two threads named here until 2026-08-31 — "transactional email does not send" and "the reminder sweep has no scheduler" — were both closed and proven in roadmap 0.2 and 0.3 on 2026-08-29. `PROJECT-STATE.md` §5 is the live list.) ~~Billing is not implemented; nothing charges anyone.~~ **Billing is BUILT and switched off as of 2026-09-05 (roadmap 2.20 stage 2): the checkout, the detailer's own billing screen, the cancel button AB 2863 requires, and what happens when a card fails. It has since been exercised end to end against real Stripe in TEST MODE (2026-09-05) — a real Checkout paid with a test card, and a test-clock run that took a tenant from past due to offline and back. Nothing charges anyone REAL because the Stripe account is not activated — `stripeConfigured()` is false, the checkout answers 503 and the screen says so. Test mode needs no activated account; `docs/setup-steps-2026-09-04.md` step 2b is the ten minutes.**
...
- **The checkout CHARGES what `/pricing` PRINTS, and one pure module does both** (`supabase/functions/_shared/platformBilling.ts`, the second copy of `pricing.js` this repo allows because a Deno bundle cannot import out of `supabase/`). `tests/platform-billing.test.mjs` ties every rung, founding and list, to the money handed to Stripe. **Every price is snapshotted onto the subscription row at checkout and never re-read** — a subscriber's price is fixed at the moment they agreed to it, and recomputing an early-exit fee from a later config turns $240 into $360.
