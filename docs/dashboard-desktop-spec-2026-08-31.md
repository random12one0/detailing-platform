...
5. ~~**Below 1024 pixels nothing changes at all.** The phone, which is where the
   ...
   **WITHDRAWN BY THE OWNER, 2026-08-31.** He read it as *the phone keeps the
   old dashboard* and answered *"the whole admin dashboard is changing both with
   desktop and phone."* **What is still true and still guaranteed: no screen in
   THIS file grows a second column below 1024px**, so nothing here can damage
   the phone. The phone's own redraw is
   `docs/dashboard-phone-pass-2026-08-31.md`, and it takes nothing from this
   file — **an earlier draft of it widened `--bp-rail` to short screens and the
   owner's portrait ruling withdrew that.** This specification is unchanged.
...
- ~~**It does not touch the phone.** Below 1024px every screen is what ships
  ...
  **CORRECTED 2026-08-31, STEP 4b.** True of this file's *columns*, which is
  what the sentence meant. The phone is redrawn in
  `docs/dashboard-phone-pass-2026-08-31.md` and **that file changes nothing
  here** — the rail stays a ≥1024px-wide thing, per the owner's ruling that a
  phone stays portrait in every orientation.
