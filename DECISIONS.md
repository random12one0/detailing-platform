...
### And what Fable 5.1 was spent on
...
site (`docs/design-directions/6-detailer-default.html`) — one worked example
derived from The Thread but for a local trade's shopfront rather than a SaaS
product page — and **not** the contract enumeration, which is analytical. Every
piece of content on that page that would really come from the dashboard carries
a `data-src="table.column"` attribute, so the page is also the visual half of §3.
