...
## Roadmap 3.1 — the tenant-site contract, and the four gaps that were three

**2026-09-05.** The item's own entry carried an instruction before it carried a
task: *"DO NOT WRITE ANOTHER PLAN."* The owner had already said it —
*"Isn't there already a plan. Follow the docs."* — so the work was to produce
the ONE thing `docs/tenant-websites.md` §3 says is still owed, *"the list roadmap
3.1 has to enumerate"*, and nothing else. That is
`docs/tenant-site-contract.md`.

**The enumeration is a two-way contract and only one direction had ever been
written down.** `docs/dashboard-feature-inventory-2026-08-31.md` §5 asked, for
each page a site might have, *"is this editable in the dashboard?"* — useful,
and the wrong half. The owner's own constraint is the other direction: *"a lot
of the features of the admin dashboard need some features on the website to
work."* So §2 of the contract is twelve implementations a site OWES, each one
written as **what silently stops working if the site omits it** — because every
single one fails quietly. The dashboard screen still works, the setting still
saves, and nothing anywhere reports that the feature reaches nobody. That is
this repo's oldest failure mode with a new surface to happen on.

**The sharpest of the twelve is the catalog (§2b), and it is a rule this repo
already has in a harder form.** A site that hard-codes prices is *a number
PRINTED is not a number CHARGED* **with the two numbers in two different
codebases** — the detailer raises a price in Catalog, the booking flow charges
the new one, and the site advertises the old one forever. Nothing can catch it,
because the two halves never meet.

### What was measured rather than assumed

`get_public_business_profile` is already almost the whole contract — one
`security definer` RPC, granted to `anon`, filtered on `status = 'active'`,
returning ten keys. Every public edge function already answers
`Access-Control-Allow-Origin: *` (`_shared/http.ts`), so **a bespoke site on
any origin can call the entire engine today with no change to it.** The
`business-media` bucket is public-read, so gallery images render cross-origin
from a plain `<img>`. **The fork line is therefore not a thing to build; it is
a thing to write down before somebody crosses it.**

### THE FOURTH GAP IN THE ROADMAP ENTRY IS WRONG, AND THE WAY IT GOT THERE IS THE LESSON

It said *"Five of the six social links cannot be typed in. `BusinessInfo.jsx`
edits `social_instagram` only."* That was true when
`dashboard-feature-inventory-2026-08-31.md` §3 recorded it and **stopped being
true on 2026-09-02**, when step 6 stage 6 added the fields — under a comment
that begins *"FOUR SOCIAL FIELDS, NOT ONE"*. It was copied out of a file dated
2026-08-31 into a roadmap entry dated 2026-09-05, three days after the fix, by a
session that was otherwise measuring everything it wrote. **A gap list copied
forward rots exactly like a count in CLAUDE.md rots**, and this repo has now
recorded that four times about numbers and once about a feature.

What is actually wrong there is smaller and more useful: `branding.social_google`
and `branding.social_yelp` are **dead columns that shadow live settings.** The
same screen edits `business_settings.google_review_url` / `yelp_review_url`
five lines further down, that pair is what the emails and the dashboard use, and
the branding pair have state and save code but **no input** — so they are always
written empty. A site must read the settings pair; the branding pair should be
dropped before somebody reads whichever they find first.

### Three gaps nobody had written down, one of them bigger than the four

- **Every customer-facing URL the platform emits is hardcoded to the
  platform.** `_shared/config.ts` builds `businessSiteUrl`, `receiptUrl`,
  `planUrl` and `plansUrl` from **one global `PLATFORM_URL`**, so a detailer on
  `coastlinedetail.com` still sends confirmation emails whose "view, change or
  cancel" link goes to detailingplatform.com. **This is the seam a CUSTOMER can
  see, in the one artifact the detailer did not write**, and it is the real
  content of roadmap 3.3 — whose own wording ("hostname→business lookup + the
  Netlify alias process") describes only the inbound half. `business_domains`
  has existed since the first tenant migration and nothing has ever read it.
- **`businesses.contact_email` is not in the profile.** The RPC returns `phone`
  and not it, so a site's contact section can print a number and no address.
  Possibly deliberate — the function's stated rule is "strictly public-safe"
  and an address in public JSON is scrapeable — but nothing says so. A question
  for the owner, not a build.
- **`track-visit`, `campaigns` and `campaign_visits` are dormant**: a deployed
  public function and two tables with no caller in `app/` and no reader
  anywhere. A tenant site is the natural caller, and calling it buys nothing
  until a dashboard screen reads it. **Recommended to stay dormant and be said
  so out loud**, so 3.2 does not wire every client site into an endpoint whose
  output nobody can see.

### The one thing put to the owner rather than decided

**Where the customer actually books.** The seven-step flow enforces exclusive
groups, per-service weekdays, vehicle-size adjustments, the condition question,
travel zones, drop-off-only periods, request mode and plan membership, and its
layout is measured to 10px of spare room at 1440x900. Rebuilding it per client
is forking the engine wearing a presentation costume. **The recommendation is
that a bespoke site LINKS to `/book/:slug`**, which already retints itself to
the tenant's accent; an iframe is the middle option and can be taken per client
later without changing anything, because both point at the same URL. **Its
consequence is stated before he agrees:** the booking flow is dark and is the
one surface a client site cannot restyle, so the default world is dark too, and
a client who wants a light site reopens `BookingBusinessContext.jsx`'s own note
— which has said *"reopen in phase 3"* since 2026-08-30.

### And what Fable 5.1 was spent on — CORRECTED THE SAME DAY

His instruction was *"maybe using fable when it's needed"*, and the roadmap
entry had already decided where: the **default visual world** for a detailer's
site — one worked example derived from The Thread. That page was built, and
**he rejected it on sight, correctly**: *"it shouldn't look exactly like our
landing page, it should genuinely be different. Different colors fonts
aesthetic… what I meant by default using our design, I more meant like the
mentality of how we do things. The scrolling, the inspo etc. Not making it look
like the landing page."*

**The mistake, precisely, so it is not made again:** his 2026-08-29 words in
`docs/tenant-websites.md` were *"I want that same **research** to be used"* —
research, not look. The roadmap entry paraphrased that as *"the kit ships a
default"*, a session read "default" as a skin, and Fable produced a faithful,
well-made copy of the wrong thing: same ground, same faces, same accent, same
skeleton, a detailer's name on our marketing page. **A default LOOK is the
cookie-cutter site he rejected on 2026-08-29** — the two halves of the repo had
been contradicting each other for a week and this was where it surfaced.

**What replaced it.** `docs/tenant-site-research-2026-09-05.md`: six real
detailers' live sites looked at in a browser (the earlier detailer research had
read three of them for MENU SHAPE and never once looked), §1 separating the
METHOD that transfers from the SKIN that never does, and §3 the content
inventory his second message asked for — *"include everything they would want,
nothing that doesn't make sense for a detailer."* That inventory corrected the
contract in four places (the vehicle-size price ladder was listed as omittable
and 5 of 6 real sites print it; `services.notes` was not required and 4 of 6
carry a disclaimer) and found an eighth gap (credentials have no column). Then
**three worked pages in `docs/tenant-sites/`, deliberately different from each
other and from us** — dark/serif/expensive, light/warm/personal, loud/
industrial/price-first. Three rather than one because one example is a
template. The rejected page was deleted rather than kept: its only lesson is
this paragraph.

### And then he answered §1c against the recommendation, which was the right call

The one decision 3.1 was holding was where a customer books. The contract
recommended LINKING OUT to `/book/:slug`, because the seven steps are the most
rule-dense surface in the product and a copy per client is a second version of
every rule. **He chose the form built into the site:** *"It's up to the
detailer's choice but I think it should be built into the website with the
detailer's website design. Like how it is on my website."*

**His site is the spec, and reading it is what made the answer obviously
right.** `reference/frontend/src/components/BookingWidget.jsx` is 1,581 lines
in the SITE's own components folder, built from the SITE's own UI kit, animated
with the site's own framer-motion, rendered inline by `App.js:73`. Not a link,
not a page, not an iframe. **The fork line moves up one level: the FORM is
presentation, the RULES are not** — and they never were client-side, because
`create-booking` recomputes every quote whatever the client sent, `validateSlot`
gates every time and the exclusion constraint is in the database. **A bespoke
form cannot mis-charge or double-book; it can only OFFER something the server
refuses**, which costs a customer their booking rather than the detailer money
and is the harder failure to see. **So 3.2 gained its biggest job — a headless
booking core**, one dependency-free module with no markup and no CSS, lifted
out of `BookingPage.jsx` and its six step components, so ten clients are ten
designs rather than ten re-derivations of the rules.

**Two things fell out of it, one of them free.** §8.2 — the light-tenant-site
to dark-booking-form handover, which was going to be a 3.2 build teaching the
flow to take a tenant's ground — **simply stopped existing**, because there is
no crossing when the form is on the tenant's own page: *a seam that needed a
feature to hide it went away when the seam did.* And contract §5 all but
emptied, because everything on it was excused by the single sentence *"consumed
inside the booking flow, which the site does not own"* — both halves of which
failed the same day. *A blanket reason covering a whole list is a reason nobody
re-examines per item.*

**And reading his site corrected a finding this same item had got wrong.**
Campaign tracking had been recorded as dormant — no caller in `app/`, nothing
reads the tables, "leave it that way". True of the platform, false of the
product it was built from: `App.js:29/54` calls `trackVisit()` on every page
load, `lib/campaign.js` auto-applies a campaign's promo code, and the old
admin had a Campaign Links screen. It is a working feature the conversion lost,
now on Phase 4's restoration list. ***"Nothing calls it" had been measured in
`app/` only, while the reference implementation of the whole product sat unread
in the same repo.***

### And then he rejected how all three LOOK, which is the finding that outlives the item

*"All 3 look very ai and not even like the vibe for detailing but it's fine for
now."* **They pass every mechanical check in this repo** — every never-default,
every contrast floor, 320, console, lite — **and still read as AI.** So **the
anti-slop floor is a list of NEVERS and a list of nevers cannot produce a
vibe**: it catches the tells of a few years ago, not the current house style of
AI design output (editorial serif, ruled rows, letter-spaced small-caps,
generous whitespace, a muted palette, stock photos in neat rectangles).

**"Fine for now" is a deferral** — the pages stay, nothing is rebuilt, and they
are NOT the taste reference for a real client's site until this is fixed.
**The cause that transfers is mine rather than the models': three agents given
ONE brief produce ONE family.** Same sections, same content, same seams, only
the paint varied — so three examples came out as three colourways of one page,
which is the template three examples existed to disprove. *Varying the palette
while fixing the skeleton does not produce variety.* **What unblocks it is his
